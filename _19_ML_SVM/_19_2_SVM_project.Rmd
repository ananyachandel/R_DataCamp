...
```{r}
remove(list = ls())
loans <- read.csv('./../SOURCE/CSV files for ML Projects/loan_data.csv')
```
...
```{r}
library(ggplot2)
ggplot(loans,aes(x=fico))  + geom_histogram(aes(fill=not.fully.paid),color='black',bins=40,alpha=0.5) + theme_bw()
```
...
You probably got some bad results! With the model classifying everything into one group! Let's tune our model to try to fix this.
